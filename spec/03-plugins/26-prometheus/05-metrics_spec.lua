...
for _, strategy in helpers.each_strategy() do
  describe("Plugin: prometheus (metrics)", function()
    ...
    it("expose Nginx connection metrics by admin API #a1.1", function()
      ...
      assert.matches('kong_nginx_current_connections{node_id="' .. UUID_PATTERN .. '",subsystem="' .. ngx.config.subsystem .. '",state="%w+"} %d+', body)
    end)
    ...
    it("increments the count of proxied requests #p1.1", function()
      ...
      helpers.wait_until(function()
        ...
        return body:find('kong_request_count{service="mock-ssl-service",route="mock-ssl-route",code="400",source="upstream",consumer=""} 1',
          nil, true)
      end)
    end)
